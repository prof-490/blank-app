...
st.write("Analyze your spending habits. Use the filters below to dynamically update your report and charts.")

# Define your data function locally so the page can access it
def extract_data():
    return [
        (date(2026, 6, 10), "Pick 'n Pay", 1000.00, False),
        (date(2026, 6, 12), "Clicks", 550.50, False),
        (date(2026, 6, 22), "Corner Cafe", 250.80, True),
        (date(2026, 10, 4), "Corner Cafe", 250.80, True)
    ]
...
# build dataframe
st.session_state.df = pd.DataFrame(rows, columns=["Date", "Store", "Total Cost", "Updated"])
...
if not df.empty:
    ...
    with rep_col1:
        st.metric("Total Expenses Logged", f"${total_spent:,.2f}")
    with rep_col2:
        st.metric("Receipt Count", total_receipts)
    chart_data = df.groupby("Date")["Total Cost"].sum().reset_index()
    st.scatter_chart(data=chart_data, x="Date", y="Total Cost")
